...
class SlotAttention(nn.Module):
    ...
    def forward(self,inputs,slots=None,nslots=None):
        ...
        inputs=self.pre_norm(self.pre_processing(inputs))
...
class CnnDecoder(nn.Module):#This is the big change, Spatial broadcast(I was an idiot)
    def __init__(self,feature_size=64,resolution=(320,480),internal_channels=32,out_channels=4):
        super().__init__()
        self.res=resolution
        self.outc=out_channels
        self.seq=nn.Sequential(
            SpatialBroadcast(resolution),
            nn.Conv2d(feature_size+2,internal_channels,5,padding=2),
            nn.LeakyReLU(),
            nn.Conv2d(internal_channels,internal_channels,5,padding=2),
            nn.LeakyReLU(),
            nn.Conv2d(internal_channels,out_channels,3,padding=1),
            nn.LeakyReLU(),
        )
    def forward(self,token):
        b,s,_=token.shape
        return self.seq(token).reshape(b,s,self.outc,self.res[0],self.res[1])
...
class SpatialBroadcast(nn.Module):
    def __init__(self,resolution):
        ...
        self.res=resolution
        h,w=resolution
        ly=torch.linspace(0,1,h)
        lx=torch.linspace(0,1,w)
        my,mx=torch.meshgrid(ly,lx,indexing='ij')
        self.register_buffer(
            "grid",
            torch.stack([my, mx], dim=0)
        )
    def forward(self,slots):
        b,s,feature_size=slots.shape
        slots=slots.view(b*s,feature_size,1,1).expand(-1,-1,self.res[0],self.res[1])
        grid=self.grid.unsqueeze(0).expand(b*s,-1,-1,-1)
        return torch.cat((slots,grid),dim=1).reshape(b*s,feature_size+2,self.res[0],self.res[1])
